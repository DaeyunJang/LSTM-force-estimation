...
def build_mlp(input_shape):
    ...
    # 2k-scale: MLP는 너무 작아지기 쉬워서 128-64-32 대신,
    # 논문 비교에서 'tiny'로 유지하려면 128-64-32도 가능하지만,
    # 여기서는 single-step 2k 기준(네 결과)과 일관되도록 128 하나만 유지하는 쪽이 안정적.
...
def build_cnn(input_shape):
    ...
    if input_shape[0] <= 1:
        ...
        # 2k-scale: 네가 측정한 2050 params 근처로 나오던 설정을 유지하려면 Dense(128)이 핵심
        h = tf.keras.layers.Dense(128, activation="relu")(h)
        y = tf.keras.layers.Dense(2)(h)
        return tf.keras.Model(x, y, name="CNN")

    # temporal conv path는 2k로 줄이기 위해 filter=16
...
def build_tcn(input_shape):
    x = tf.keras.Input(shape=input_shape)
    # 2k-scale: channel=16
    h = tf.keras.layers.Conv1D(16, 3, padding="causal", dilation_rate=1, activation="relu")(x)
    h = tf.keras.layers.Conv1D(16, 3, padding="causal", dilation_rate=2, activation="relu")(h)
    h = tf.keras.layers.Conv1D(16, 3, padding="causal", dilation_rate=4, activation="relu")(h)
    h = tf.keras.layers.GlobalAveragePooling1D()(h)
    y = tf.keras.layers.Dense(2)(h)
...
def build_lstm(input_shape):
    x = tf.keras.Input(shape=input_shape)
    # 2k-scale: (H1,H2)=(12,6) + Dense(32) 유지 (원본 구조 유지)
    h = tf.keras.layers.LSTM(12, return_sequences=True)(x)
    h = tf.keras.layers.LSTM(6)(h)
...
def build_gru(input_shape):
    x = tf.keras.Input(shape=input_shape)
    # 2k-scale: (H1,H2)=(16,8) (원본 구조 유지)
    h = tf.keras.layers.GRU(16, return_sequences=True)(x)
    h = tf.keras.layers.GRU(8)(h)
    y = tf.keras.layers.Dense(2)(h)
...
def build_transformer(input_shape):
    x = tf.keras.Input(shape=input_shape)
    # 2k-scale: d_model=8, heads=1, ff=16 (원본 구조 유지)
    d_model = 8
    h = tf.keras.layers.Dense(d_model)(x)

    attn = tf.keras.layers.MultiHeadAttention(num_heads=1, key_dim=d_model)(h, h)
    h = tf.keras.layers.Add()([h, attn])
    h = tf.keras.layers.LayerNormalization()(h)

    ff = tf.keras.layers.Dense(16, activation="relu")(h)
...
def build_convmixer(input_shape):
    ...
    if input_shape[0] <= 1:
        h = tf.keras.layers.Flatten()(x)
        # 2k-scale: Dense(128) 유지 (네 single-step 2k 결과와 일관)
        h = tf.keras.layers.Dense(128, activation="relu")(h)
        y = tf.keras.layers.Dense(2)(h)
        return tf.keras.Model(x, y, name="ConvMixer")

    # temporal conv path는 2k로 줄이기 위해 16
    h = tf.keras.layers.Conv1D(16, 3, padding="same", activation="relu")(x)
    h = tf.keras.layers.DepthwiseConv1D(3, padding="same", activation="relu")(h)
    h = tf.keras.layers.Conv1D(16, 1, padding="same", activation="relu")(h)
    h = tf.keras.layers.GlobalAveragePooling1D()(h)
    y = tf.keras.layers.Dense(2)(h)
    return tf.keras.Model(x, y, name="ConvMixer")
...
def build_kalmannet_lite(input_shape):
    ...
    # (SEQ_LEN, F) -> hidden -> (2)
    x = tf.keras.Input(shape=input_shape)
    # 2k-scale: GRU(16) + Dense(16)
    h = tf.keras.layers.GRU(16)(x)  # temporal encoder
