...
def main():
    ...
    # --- Joint Angle 미사용 강제 적용 ---

    # meta.json에 Joint Angle_*가 남아있어도, 입력에서는 무시한다.

    input_columns = [c for c in input_columns if not c.startswith("Joint Angle")]


    # 원하는 입력 4개만 사용 (모델 입력 순서 중요)

    desired_inputs = ["wire length #0", "wire length #1", "loadcell #0", "loadcell #1"]

    input_columns = desired_inputs

    col_size = int(train_meta.get("data", {}).get("joint_angle_len_expected", 0))
    ...
    for csv_path, json_path in zip(test_csv, test_json):
        ...
        # Joint Angle 미사용: 필요한 컬럼만 남기고 NaN 제거
        # 필요한 컬럼만 기준으로 NaN 제거 (혹시 CSV에 컬럼이 없으면 KeyError 방지)

        subset_cols = [c for c in (input_columns + output_columns) if c in df.columns]

        missing_cols = [c for c in (input_columns + output_columns) if c not in df.columns]

        if missing_cols:

            print(f"[WARN] Missing columns in CSV (ignored in dropna): {missing_cols}")
