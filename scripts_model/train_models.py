...
def detect_joint_angle_len(json_path):
    tmp = pd.read_json(json_path)
    return int(tmp["Joint Angle"].apply(safe_len).mode().iloc[0])
...
def main():
    ...
    # joint angle length & columns
    col_size = detect_joint_angle_len(json_files[0])

    # load df for each file and expand joint angles
    ...
    expanded_csv_files = []
    expanded_json_files = []
    # 실제로는 파일 경로만 유지하고 build_sequences에서 df를 다시 읽으므로,
    # 여기서 joint angle 펼치기를 build_sequences에서 수행하도록 하는게 정석인데,
    # 현재는 빠르게 안정화 목적이라: build_sequences 전에 조인/펼치기를 포함시키지 않고,
    # 대신 파일에서 json을 그대로 concat 후 "Joint Angle" 컬럼을 펼치는 로직을 build_sequences에 넣어야 함.
    # -> 여기선 build_sequences를 수정해서 처리한다.

    # build sequences with expansion inside
    def build_sequences_with_joint_expansion(csv_files, json_files):
        X_list, y_list = [], []
        mismatch_logs = []
        for csv_path, json_path in zip(csv_files, json_files):
            df_csv = pd.read_csv(csv_path).reset_index(drop=True)
    ...
    X, y, mismatch_logs = build_sequences_with_joint_expansion(csv_files, json_files)
    if X.shape[0] == 0:
    ...
    meta = {
        ...
        "data": {
            ...
            "joint_angle_len_expected": int(col_size),
        },
        "shapes": {
            "X": list(X.shape),
            "y": list(y.shape),
