...
class ExternalForceEstimationNode(Node):
    ...
    def __init__(self):
        ...
        model_path = os.path.join(package_share_directory, "resnet_model.h5")
        ...
        self.scaler_x_path = (
            self.get_parameter("scaler_x_path").get_parameter_value().string_value
        )

        scaler_y_path = os.path.join(package_share_directory, "scaler_y.pkl")
        self.declare_parameter("scaler_y_path", scaler_y_path)
        self.scaler_y_path = (
            self.get_parameter("scaler_y_path").get_parameter_value().string_value
        )
        ...
        print(
            f"[external_force_estimation.py] lstm_model_legacy.h5 PATH: {model_path}",
            flush=True,
        )
        print(
            f"[external_force_estimation.py] scaler_x.pkl PATH: {scaler_x_path}",
            flush=True,
        )
        print(
            f"[external_force_estimation.py] scaler_y.pkl PATH: {scaler_y_path}",
            flush=True,
        )
        ...
        self.lc_subscriber = self.create_subscription(
            LoadcellState, "loadcell_state", self.read_loadcell_data, QOS_RKL1V
        )
        ...
        self.wire_length_subscriber = self.create_subscription(
            Float64MultiArray, "wire_length", self.read_wire_length, QOS_RKL1V
        )
        ...
        self.external_force_publisher = self.create_publisher(
            Vector3, "estimated_external_force", QOS_RKL1V
        )
    ...
    def estimation_process(self):
        ...
        while rclpy.ok():
            try:
                # self.get_logger().info(f"EXT force estimating.")
                ...
                input = np.concatenate((wl, lc, segment_angle))  # (13,)
                # self.get_logger().info(f"mode input size: {input.shape}")
                input_for_model = input.reshape(1, -1)  # (1,13)
                # self.get_logger().info(f"mode input_for_model size: {input_for_model.shape}")
                input_for_model_normalized = self.scaler_x.transform(input_for_model)
                # self.get_logger().info(f"mode input_for_model_norm size: {input_for_model_normalized.shape}")
                # input_for_model_final = np.expand_dims(
                #     input_for_model_normalized, axis=-1
                # )  # (1,13,1)
                input_for_model_final = input_for_model_normalized.reshape(1, 1, -1).astype(np.float32)  # (None, 13, 1)
                # input_name = self.model.input_names[0]
                # self.get_logger().info(f"mode input_for_model_final size: {input_for_model_final.shape}")
                predicted_normalized = self.model({"input_layer": input_for_model_final})
                predicted_denormalized = self.scaler_y.inverse_transform(
