...
# data_csv = sorted(glob('../datasets_0102_offset_30deg/train/data_LPF_*.csv'))
data_csv = sorted(glob('../datasets_0102_offset_30deg/train/data_*.csv'))
joint_angle_json = sorted(glob('../datasets_0102_offset_30deg/train/curve_fit_result-joint_angle_*.json'))
...
save_dir = os.path.join('..', 'fit', 'fit_PINNs')
...
def pinn_loss(y_true, y_pred):
    physics_loss = tf.reduce_mean(tf.square(y_pred - y_true))  # 물리적 제약 추가 가능
    return physics_loss

model = tf.keras.Sequential([
    tf.keras.layers.Input(shape=(x_train.shape[1], 1)),
    tf.keras.layers.Flatten(),  # 1D로 변환
...
# 모델 저장
model.save(os.path.join(save_dir, 'PINNs_model.h5'))
