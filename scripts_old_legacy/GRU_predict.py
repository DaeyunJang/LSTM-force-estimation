...
model_dir = os.path.join('..', 'fit', 'fit_GRU', '20260115-093758')
save_dir = os.path.join('..', 'results', 'results_GRU')
save_dir = os.path.join(save_dir, time_now)
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

# 모델 불러오기
model = tf.keras.models.load_model(os.path.join(model_dir, 'GRU_model.h5'))
...
# 병합된 테스트용 CSV와 JSON 데이터를 하나의 데이터                                                                                                                                                                            프레임으로 병합합니다.
