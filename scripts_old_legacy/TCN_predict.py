...
model_dir = os.path.join('..', 'fit', 'fit_TCN', '20260115-094244')
save_dir = os.path.join('..', 'results', 'results_TCN')
save_dir = os.path.join(save_dir, time_now)
if not os.path.exists(save_dir):
    os.makedirs(save_dir)
# 모델 불러오기
# model = tf.keras.models.load_model(os.path.join(model_dir, 'TCN_model.h5'))
model = tf.keras.models.load_model(os.path.join(model_dir, 'TCN_model.h5'), custom_objects={'TCN': TCN})
...
test_csv = sorted(glob('../datasets_0102_offset_30deg/test/data_*.csv'))
test_json = sorted(glob('../datasets_0102_offset_30deg/test/curve_fit_result-joint_angle_*.json'))
...
#############################################################################
# # 각 리스트의 길이 계산
# lengths = [len(x) for x in test_data_expanded['Joint Angle']]
#
